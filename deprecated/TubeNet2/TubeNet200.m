...
S = fixeye2(M,S_sz,M_sz,1);
% imagesc(reshape(S,S_sz));
rollingR = 1;


net = feedforwardnet(H_sz);
net.trainParam.showWindow=0;
net.trainParam.epochs=1;
net.trainParam.min_grad = 0;
% Snet.performParam.regularization = 0.1;

Snet = adapt(net,rand([prod(M_sz)+prod(S_sz) 2]),rand([prod(S_sz) 2])); %first one has to contain 2+ training examples for some reason..
Mnet = adapt(net,rand([prod(M_sz)+prod(S_sz) 2]),rand([prod(M_sz) 2])); %first one has to contain 2+ training examples for some reason..
...
for iters = 0:500
    ...
    Snew = fixeye2(M,S_sz,M_sz,iters);
