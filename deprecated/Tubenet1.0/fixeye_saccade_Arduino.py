#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 11:18:01 2017
@author: jordandekraker
"""
import cv2
import numpy as np
import scipy.misc
import matplotlib.pyplot as plt
        
#Mout = [-1., -1., -0.]

# fisheye filter
def fixeye(Mout):
    image = cv2.imread("./SmileyFace8bitGray.png",cv2.IMREAD_GRAYSCALE)
    image = image.astype(float)
    image = image+0.1 # otherwise can lead to cropping problems in Mouteye
    outsz = [32,32]
#    Mout = Mout.astype(float)
    
    # make sure input is square matrices
    image = np.reshape(image,[np.int(np.sqrt(image.size)),np.int(np.sqrt(image.size))])
    
    #convert Mout to coordinates in a given range
    Mout[0] = np.round(Mout[0]*image.shape[0]/2 + (image.shape[0]/2))
    Mout[1] = np.round(Mout[1]*image.shape[1]/2 + (image.shape[0]/2))
    Mout[2] = np.round(10.**(Mout[2]+2)) # range 10-1000
    
    
    # set up fisheye parameters
    cam = np.eye(3)
    cam[0,2] = Mout[0]  # define center x
    cam[1,2] = Mout[1]  # define center y
    cam[0,0] = Mout[2]        # define focal length x
    cam[1,1] = Mout[2]        # define focal length y
    #run fisheye
    dst = cv2.undistort(image,cam,1)
    
#    #crop
#    dst = dst[~np.all(dst==0,axis=0)]
#    dst = dst.T
#    dst = dst[~np.all(dst==0,axis=1)]
#    dst = dst.T

    # resize and normalize
    dst = scipy.misc.imresize(dst,outsz)
    dst = np.reshape(dst,[outsz[0]*outsz[1]]) #make 1D
    dst = dst - np.mean(dst)
#    dst = dst / np.std(dst)
    return dst #.astype('float32')

#dst = fixeye(Mout)
#dst = np.reshape(dst,[32,32])
#fig, ax = plt.subplots()
#ax.imshow(dst)